...
public final class Transients {
	...
	private static final byte BOOLEAN_OFFSET_USER_PIN_MODE_83 = BOOLEAN_OFFSET_USER_PIN_MODE_82 + 1;
	private static final byte BOOLEAN_OFFSET_SECURE_MESSAGING_OK = BOOLEAN_OFFSET_USER_PIN_MODE_83 + 1;
	private static final byte BOOLEANS_SIZE = BOOLEAN_OFFSET_SECURE_MESSAGING_OK + 1;

	protected Transients() {
		buffer = JCSystem.makeTransientByteArray(Constants.INTERNAL_BUFFER_MAX_LENGTH, JCSystem.CLEAR_ON_RESET);
		shorts = JCSystem.makeTransientShortArray(SHORTS_SIZE, JCSystem.CLEAR_ON_RESET);
		bytes = JCSystem.makeTransientByteArray(BYTES_SIZE, JCSystem.CLEAR_ON_RESET);
		booleans = JCSystem.makeTransientBooleanArray(BOOLEANS_SIZE, JCSystem.CLEAR_ON_RESET);
	}
	...
	protected final void setUserPinMode83(final boolean is83) {
		booleans[BOOLEAN_OFFSET_USER_PIN_MODE_83] = is83;
	}

	protected final boolean userPinMode83() {
		return booleans[BOOLEAN_OFFSET_USER_PIN_MODE_83];
	}
